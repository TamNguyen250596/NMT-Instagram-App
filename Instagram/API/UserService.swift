...
struct UserService {
    ...
    static func follow(uid: String, completion: @escaping CompletionHandlerOptionalErrorToVoid) {
        ...
        Collection_Following.document(cuurentUid).collection(User_Following).document(uid).setData([:], completion: { (error) in
            
            Collection_Followers.document(uid).collection(User_Followers).document(cuurentUid).setData([:], completion: completion)
        })
    }
    ...
    static func unfollow(uid: String, completion: @escaping CompletionHandlerOptionalErrorToVoid) {
        ...
        Collection_Following.document(cuurentUid).collection(User_Following).document(uid).delete(completion: { (error) in
            
            Collection_Followers.document(uid).collection(User_Followers).document(cuurentUid).delete(completion: completion)
        })
    }
    ...
    static func checkIfUserIsFollowed(uid: String, completion: @escaping CompletionHandlerBoolToVoid) {
        ...
        Collection_Following.document(cuurentUid).collection(User_Following).document(uid).getDocument(completion: { (snapshot, error) in
